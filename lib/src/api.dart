...
/* production queries are sent using base64 encript */
    ...
    QueryResult? result = await Query(
            lat: instance.position?.latitude.toString(),
            lon: instance.position?.longitude.toString(),
    ...
    if (result?.getStatus() != Httpr.ok) {
      ...
      /* Returns error and logouts if user is not a driver */

      if (doLogin && result.geterror() == Protocol.noDriver) {
...
class Query {
  ...
  Future<QueryResult?> run() async {
    ...
    if (lat != null && lat!.isNotEmpty) {
      request.options.headers['lat'] = lat;
    }

    if (lon != null && lon!.isNotEmpty) {
      request.options.headers['lon'] = lon;
    }
